...
function isInAppBrowser() {
    ...
    return /Snapchat|Instagram|FBAN|FBAV|FB_IAB|TikTok|musical_ly|Twitter/.test(ua) ||
...
export default function AddClient() {
    ...
    useEffect(() => {
        ...
        if (!inApp) {
            // Safari / Chrome — auto-redirect works fine
            window.location.href = `phantomtrack://add?uid=${uid}`;
        }

        // Show the button after a short delay so the hero text animates in
        const t = setTimeout(() => setReady(true), inApp ? 300 : 2000);

        return () => { clearTimeout(t); document.head.removeChild(meta); };
    }, [uid, router]);

    const handleOpen = () => {
        if (!uid) return;
        // User gesture — WebKit allows custom URL scheme navigation
        window.location.href = `phantomtrack://add?uid=${uid}`;
    };
                ...
                <AnimatePresence>
                    {ready && (
                        ...
                        >
                            ...
                            <button
                                onClick={handleOpen}
                                className="group relative w-full inline-flex items-center justify-center gap-2 font-bold tracking-wide overflow-hidden transition-all px-6 py-5 bg-primary text-white rounded-2xl text-base active:scale-[0.98] cursor-pointer"
                            >
                                ...
                                <span className="relative">Open in Phantom Track</span>
